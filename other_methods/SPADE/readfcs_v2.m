...
elseif strcmp(fcsheader_type,'FCS3.1')% FCS2.0 or FCS3.0 or 3.1 types
    fcshdr.fcstype      = fcsheader_type;
    FcsTextStartPos     = str2num(char(fcsheader(11:18)'));  % numbers/coordinates defined by FCS3.0 standard
    FcsTextStopPos      = str2num(char(fcsheader(19:26)'));
    FcsDataStartPos     = str2num(char(fcsheader(27:34)'));   
    ...
    if fcstext_main(1) == 12
        mnemonic_separator = 'FF';
	elseif fcstext_main(1) == 9
        mnemonic_separator = char(fcstext_main(2));
	else
        mnemonic_separator = char(fcstext_main(1));
    end
    if mnemonic_separator == '@'
    ...
    fcshdr.NumOfPar = str2num(get_mnemonic_value31('$PAR',fcstext_main, mnemonic_separator));
    for i=1:fcshdr.NumOfPar
        fcshdr.par(i).bit = str2num(get_mnemonic_value31(['$P',num2str(i),'B'],fcstext_main, mnemonic_separator));
        fcshdr.par(i).display = get_mnemonic_value31(['$P',num2str(i),'D'],fcstext_main, mnemonic_separator);
        fcshdr.par(i).name = get_mnemonic_value31(['$P',num2str(i),'N'],fcstext_main, mnemonic_separator);
        fcshdr.par(i).name2 = get_mnemonic_value31(['$P',num2str(i),'S'],fcstext_main, mnemonic_separator);
        fcshdr.par(i).range = str2num(get_mnemonic_value31(['$P',num2str(i),'R'],fcstext_main, mnemonic_separator));
        
        % linear scaling factor
        tmp = get_mnemonic_value31(['$P',num2str(i),'G'],fcstext_main, mnemonic_separator);
        if isempty(tmp) || isequal(str2num(tmp),0)
            fcshdr.par(i).G = 1;
        else
            fcshdr.par(i).G = str2num(tmp);
        ...
        % this part of code handles the display option, log/linear transformation, scale and limit of axes, this is useless
        par_exponent_str= (get_mnemonic_value31(['$P',num2str(i),'E'],fcstext_main, mnemonic_separator));
        if isempty(par_exponent_str)
            % There is no "$PiE" mnemonic in the Lysys format
            % in that case the PiDISPLAY mnem. shows the LOG or LIN definition
            islogpar = get_mnemonic_value31(['P',num2str(i),'DISPLAY'],fcstext_main, mnemonic_separator);
            if length(islogpar)==3 && isequal(islogpar, 'LOG')  % islogpar == 'LOG'
               par_exponent_str = '5,1'; 
            else % islogpar = LIN case
                par_exponent_str = '0,0';
    ...
    fcshdr.TotalEvents = str2num(get_mnemonic_value31('$TOT',fcstext_main, mnemonic_separator));
    fcshdr.byteorder = get_mnemonic_value31('$BYTEORD',fcstext_main, mnemonic_separator);          % this influences the reading of the data
    fcshdr.datatype = get_mnemonic_value31('$DATATYPE',fcstext_main, mnemonic_separator);
    fcshdr.datamode = get_mnemonic_value31('$MODE',fcstext_main, mnemonic_separator);              % this can be L, C, U, refer to FCS3.0 standard for their meanings
    if ~isequal(upper(fcshdr.datamode(1)),'L')
        display('This reader only handles listmode format. Files with histograms cannot be read here~');
        return
    end
    
    fcshdr.starttime = get_mnemonic_value31('$BTIM',fcstext_main, mnemonic_separator);
    fcshdr.stoptime = get_mnemonic_value31('$ETIM',fcstext_main, mnemonic_separator);
    fcshdr.date = get_mnemonic_value31('$DATE',fcstext_main, mnemonic_separator);
    fcshdr.cytometry = get_mnemonic_value31('$CYT',fcstext_main, mnemonic_separator);
    fcshdr.system = get_mnemonic_value31('$SYS',fcstext_main, mnemonic_separator);
    fcshdr.project = get_mnemonic_value31('$PROJ',fcstext_main, mnemonic_separator);
    fcshdr.experiment = get_mnemonic_value31('$EXP',fcstext_main, mnemonic_separator);
    fcshdr.cells = get_mnemonic_value31('$Cells',fcstext_main, mnemonic_separator);
    fcshdr.creator = get_mnemonic_value31('CREATOR',fcstext_main, mnemonic_separator);
    
    % deal with the compensation matrix or spill over matrix 
    tmp = get_mnemonic_value31('$COMP',fcstext_main, mnemonic_separator);
    if ~isempty(tmp)
        display(['This reader cannot handle compensation matrix under $COMP keyword, but can handle SPILL']);
        return
    end
    tmp1 = [];
	tmp2 = [];
%     tmp1 = get_mnemonic_value31('SPILL',fcstext_main, mnemonic_separator);
%     tmp2 = get_mnemonic_value31('SPILLOVER',fcstext_main, mnemonic_separator);
    tmp3 = get_mnemonic_value31('$SPILLOVER',fcstext_main, mnemonic_separator);
%     if double(~isempty(tmp1)) + double(~isempty(tmp2)) + double(~isempty(tmp2))>=2
%         error('More than one of the keywords SPILL/SPILLOVER/$SPILLOVER appear in TEXT segment');
%     end
    tmp = [tmp1,tmp2,tmp3];
    if ~isempty(tmp)
        ...
        spillover_matrix = str2num(fcshdr.SPILLOVER(seg_start(1+num_compensated_channels+1):findstr(char(tmp(1:end)),'@')-2));
        spillover_matrix = reshape(spillover_matrix,num_compensated_channels,num_compensated_channels)';
        [C,IA,IB] = intersect(channel_names,compensated_channel_names);
        fcshdr.spillover_matrix = eye(length(channel_names));
        fcshdr.spillover_matrix(IA,IA) = spillover_matrix(IB,IB);
...
if isequal(FcsDataStartPos,0) || isempty(FcsDataStartPos), % if the data start position is not stored in the beginning of the header, search for the keywords in the header content
    if strcmp(fcsheader_type,'FCS2.0') || strcmp(fcsheader_type,'FCS3.0')
	...
	elseif strcmp(fcsheader_type,'FCS3.1')
		FcsDataStartPos=str2num(get_mnemonic_value31('$BEGINDATA',fcstext_main, mnemonic_separator));
		FcsDataStopPos =str2num(get_mnemonic_value31('$ENDDATA',fcstext_main, mnemonic_separator));
	end
...
elseif strcmp(fcsheader_type,'FCS3.0')
    if strcmp(mnemonic_separator,'|') || strcmp(mnemonic_separator,'$')  % CyAn Summit FCS3.0
        if fcshdr.par(1).bit == 16
            if isequal(str2num(fcshdr.byteorder),[1,2,3,4])
...
function mneval = get_mnemonic_value31(mnemonic_name,fcsheader,mnemonic_separator)
...
%     mnemonic_startpos(char(fcsheader(mnemonic_stoppos))~=mnemonic_separator | char(fcsheader(mnemonic_startpos-1))~=mnemonic_separator)=[];
    ...
    mnemonic_stoppos = mnemonic_startpos + mnemonic_length;
    next_slashes = findstr(char(fcsheader(mnemonic_stoppos+1:end)'),mnemonic_separator);
	if isempty(next_slashes), next_slashes = findstr(char(fcsheader(mnemonic_stoppos+1:end)'),'@'); end
	next_slash = next_slashes(1) + mnemonic_stoppos;
    mneval = char(fcsheader(mnemonic_stoppos+1:next_slash-1)');
	if mneval(end) == 9, mneval = mneval(1:end-1); end
elseif strcmp(mnemonic_separator,'FF')
    mnemonic_startpos = findstr(char(fcsheader'),mnemonic_name);
    if isempty(mnemonic_startpos)
        mneval = [];
