...
elseif strcmp(fcsheader_type,'FCS3.1') % FCS2.0 or FCS3.0 types
	...
	elseif fcsheader_main(1) == 9
        mnemonic_separator = char(fcsheader_main(2));
    else
        mnemonic_separator = char(fcsheader_main(1));
    end
    if mnemonic_separator == '@';% WinMDI
    ...
    fcshdr.TotalEvents = str2num(get_mnemonic_value31('$TOT',fcsheader_main, mnemonic_separator));
    fcshdr.NumOfPar = str2num(get_mnemonic_value31('$PAR',fcsheader_main, mnemonic_separator));
    fcshdr.Creator = get_mnemonic_value31('CREATOR',fcsheader_main, mnemonic_separator);
    for i=1:fcshdr.NumOfPar
        fcshdr.par(i).name = get_mnemonic_value31(['$P',num2str(i),'N'],fcsheader_main, mnemonic_separator);
        fcshdr.par(i).name2 = get_mnemonic_value31(['$P',num2str(i),'S'],fcsheader_main, mnemonic_separator);
        fcshdr.par(i).range = str2num(get_mnemonic_value31(['$P',num2str(i),'R'],fcsheader_main, mnemonic_separator));
        fcshdr.par(i).bit = str2num(get_mnemonic_value31(['$P',num2str(i),'B'],fcsheader_main, mnemonic_separator));
%==============   Changed way that amplification type is treated ---  ARM  ==================
        par_exponent_str= (get_mnemonic_value31(['$P',num2str(i),'E'],fcsheader_main, mnemonic_separator));
        if isempty(par_exponent_str)
            % There is no "$PiE" mnemonic in the Lysys format
            % in that case the PiDISPLAY mnem. shows the LOG or LIN definition
            islogpar = get_mnemonic_value31(['P',num2str(i),'DISPLAY'],fcsheader_main, mnemonic_separator);
            if length(islogpar)==3 && isequal(islogpar, 'LOG')  % islogpar == 'LOG'
               par_exponent_str = '5,1'; 
            else % islogpar = LIN case
                par_exponent_str = '0,0';
...
%============================================================================================
    ...
    fcshdr.starttime = get_mnemonic_value31('$BTIM',fcsheader_main, mnemonic_separator);
    fcshdr.stoptime = get_mnemonic_value31('$ETIM',fcsheader_main, mnemonic_separator);
    fcshdr.cytometry = get_mnemonic_value31('$CYT',fcsheader_main, mnemonic_separator);
    fcshdr.date = get_mnemonic_value31('$DATE',fcsheader_main, mnemonic_separator);
    fcshdr.byteorder = get_mnemonic_value31('$BYTEORD',fcsheader_main, mnemonic_separator);
    fcshdr.datatype = get_mnemonic_value31('$DATATYPE',fcsheader_main, mnemonic_separator);
    fcshdr.system = get_mnemonic_value31('$SYS',fcsheader_main, mnemonic_separator);
    fcshdr.project = get_mnemonic_value31('$PROJ',fcsheader_main, mnemonic_separator);
    fcshdr.experiment = get_mnemonic_value31('$EXP',fcsheader_main, mnemonic_separator);
    fcshdr.cells = get_mnemonic_value31('$Cells',fcsheader_main, mnemonic_separator);
    fcshdr.creator = get_mnemonic_value31('CREATOR',fcsheader_main, mnemonic_separator);
else
...
if strcmp(mnemonic_separator,'\')  || strcmp(mnemonic_separator,'!') ...
    ...
    mnemonic_length = length(mnemonic_name);
    mnemonic_stoppos = mnemonic_startpos(1) + mnemonic_length;
    next_slashes = findstr(char(fcsheader(mnemonic_stoppos+1:end)'),mnemonic_separator);
