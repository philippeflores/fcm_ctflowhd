...
% Last Modified by GUIDE v2.5 01-Mar-2022 08:48:47
...
if length(directoryname)==0 || isequal(directoryname,0) % the user may click cancel in the selection dialog box
    ...
    if isempty(handles.directoryname)
        fprintf('Current directory is empty.\n\n');
    else
...
if exist(parameter_filename)==2 % if a SPADE parameter file and it passed the above check (all_fcs_filenames match)
    ...
    handles.max_allowable_events = 50000;
    handles.number_of_desired_clusters = 100;
    handles.clustering_algorithm = 'kmeans'; % possible choices are 'kmeans', 'agglomerative'
    handles.file_used_to_build_SPADE_tree = cell(0);
...
%             clear('fcsdat');
    ...
    [local_density] = compute_local_density(data(IA,:), kernel_width, optimizaiton_para);
...
if size(data,2)>handles.max_allowable_events
    display(['Since the number of cells exceeds the max number of allowable events ', num2str(handles.max_allowable_events),', uniform downsampling is performed']);
...
save(fullfile(handles.directoryname, handles.pooled_downsampled_filename), 'data', 'local_density', 'marker_names', 'used_markers');
fprintf('Done!\n\n');
guidata(hObject, handles);
end
...
switch handles.clustering_algorithm
    case 'kmeans'    
        ...
        if handles.number_of_desired_clusters==size(data(IA,:),2)
            centers = data(IA,:); idx = 1:size(data(IA,:),2);
        else
            [idx, centers] = kmeans_phase1(data(IA,:)', handles.number_of_desired_clusters); centers = centers'; idx = idx';
        end
        [mst_tree,adj2] = mst(centers');
        fprintf('Done\n\n');
    case 'agglomerative'    
        % clustering and mst using agglomerative
        [mst_tree, idx] = SPADE_cluster_cells(data(IA,:), handles.number_of_desired_clusters);
        data(:,idx==0)=[];
        local_density(idx==0)=[];
        idx(idx==0)=[];
    otherwise
...
for i=1:length(handles.all_fcs_filenames)
    ...
    [min_dist,NN_index] = compute_min_dist_upsample(data(IA2,:),clustered_data(IA1,:));
...
function button_4_mst_layout_Callback(hObject, eventdata, handles), end

% --- Executes on button press in button_4_upsampling.
function button_4_upsampling_Callback(hObject, eventdata, handles), end
...
function button_one_click_for_all_Callback(hObject, eventdata, handles)
	...
	computeSpade = toc;
	fprintf("Temps de calcul SPADE : %f s\n",computeSpade)
end
