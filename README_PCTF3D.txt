...
visualization steps are included in other scripts ('CTFlowHD_plot_*.m').
************************************************
...
*******************  Step #5  ****************** >>> Lines 15 to 22
Select the variables to run PCTF3D. At line 17, this is possible to 
...
*******************  Step #6  ****************** >>> Lines 23 to 29
This section permits to define the discrete intervals for each of the 
...
interval [a,b] for each variable in *indVar*. The principle of 
'supportDist'  is to restrain a large interval [a0,b0] into a fitted 
interval [a,b]. Hence, *a0* must be defined such that a0<a and *b0* such 
that b<b0.

For fluorescence variables, the values *a0* and *b0* are called *minFluo* 
and *maxFluo*. Those values are set by default to [-5,10]. To change those 
values to *newMin* and *newMax*, enter those parameters into the function:
    >>> newMin = -1; newMax = 6;
    >>> t = supportDist(X,indVar,100,'minFluo',newMin,'maxFluo',newMax);

For scatter variables, the values *a0* and *b0* are called *minScat* and 
*maxScat* and are set by default to [0,1e+6]. To change those values, enter
those parameters into the function:
...
setting the parameter 'strMethod'. This parameter is set to 'none' but it 
can be change to 'fluo', 'label'  or 'man'.
    - 'none' (default):
...
variables, the interval is fitted from [minFluo, maxFluo]. 
    - 'label':
This option is suited if *indVar* contains both scatter and fluorescence
...
its type (fluorescence or scatter). See below for more information.
    - 'man':
...
Hence, it can be added as an input of supportDist:
    >>> t = supportDist(X,indVar,100,'strMethod','label', ...
            'strLabel',strLabel);

The option 'edges' is mandatory for the 'strMethod' option 'man'. This
variable must be of type cell of size 1xM where *M* is the number of 
variables in *indVar*. The m-th element edges{m} of *edges* must be a 1x2 
vector containing the values [a,b] of the corresponding m-th variable of 
*indVar*. After creating such variable *edges*, it can be added as an input
of 'supportDist':
    >>> edges = cell(1,M);
    >>> edges{1} = [...]; ... 
...
After the subset of triplets *calT* is defined, the resulted
3D marginals are computed. In addition to the computation of the 3D 
marginals of the coupling, all marginals can be computed if the error on 
all possible marginals must be computed. For datasets with a lot of 
variables, it is possible not to compute all possible marginals. To do so, 
you can comment the lines 35 and 36.
...
[M 1 2]. Between two triplets, the operation +1 is applied.
    ...
    >>> [calT,T] = createTriplets(M,indVar,strStrategy);
    - '+2' :
$\lfloor M/2 \rfloor$ triplets are considered. The first triplet is 
[1 2 3], the second is [3 4 5], then [5 6 7], and so on until [M-2 M-1 M] 
if *M* is odd and [M-3 M-2 M-1], [M-2 M-1 M] if *M* is even. Between two 
triplets, the operation +2 is applied.
    >>> strStrategy = '+2';
    >>> [calT,T] = createTriplets(M,indVar,strStrategy);
    - 'rng' :
...
number is defines by round(3*T/M) where *T* is the number of triplets and 
*M* is the number of variables in *indVar*. This strategy requires that the
...
*******************  Step #8  ****************** >>> Lines 45 to 56
This section builds the NBM of *X* thanks to the PCTF3D algorithm. The only
mandatory argument of this algorithm is the number of NBM components *R*.
...
that contains the *M* factors matrices where *M* is the number of variables
in *indVar*. Therefore, *y0* must be a 1xM cell and each y0{m} must be a 
IxR matrix.
    >>> y0 = ...;
...
components, it is possible to change the parameter *threshLambda*.
    >>> threshLambda = 1/(20*R);
    >>> [y,lambda,~,~,~,~,R] = PCTF3D(dataMarg,R, ...
            'threshLambda',threshLambda);
    - 'computeError' (default --> 0) :
This option permits to choose if or not the estimation errors are computed.
    >>> computeError = 1;
    >>> [y,lambda,cost,error3,error1] = PCTF3D(dataMarg,R, ...
            'computeError',computeError);
************************************************

*******************  Step #9  ****************** >>> Lines 57 to the end
...
except *dataMarg*, *dataMargFull* and *X* which are too big for storage. 
The name of the saved file will be set as follows : for the example of 
'file_CCT' with I=25, R=10 and T=4 the filename will be 
'file_CCT_I25R10T4.mat'.
...
To save the observation matrix *X*:
    >>> savePCTF3D('data') 
To save the marginals *dataMarg* and *dataMargFull* (if it exists):
    >>> savePCTF3D('marg')
